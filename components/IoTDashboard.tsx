    ...
    return (
      <div className="bg-white p-6 rounded-2xl border border-black/5 shadow-sm min-h-[400px] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4 text-slate-400">
          <p className="text-sm font-black uppercase animate-pulse">Initializing Sensors...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-white p-6 rounded-2xl border border-black/5 shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 font-bold">Real-time soil & environmental monitoring</p>
        </div>
        <button
          onClick={() => setIsSimulating(!isSimulating)}
          className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors ${isSimulating ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'
            }`}
        >
          {isSimulating ? 'Live Syncing' : 'System Paused'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className={`text-[10px] font-black uppercase tracking-widest ${moistureWarning ? 'text-red-600' : 'text-blue-600'}`}>
              Soil Moisture
            </span>
            {moistureWarning && <span className="text-[10px] font-black text-red-600 animate-pulse">ALERT</span>}
          </div>
          <div className="text-3xl font-black text-slate-800 tracking-tighter">{current.moisture}%</div>
          <p className={`text-[10px] mt-1 font-bold uppercase ${moistureWarning ? 'text-red-500' : 'text-slate-500'}`}>
            {moistureWarning ? 'Critical level' : 'Optimal range'}
          </p>
        </motion.div>
        ...
        >
          ...
          <div className="text-3xl font-black text-slate-800 tracking-tighter">{current.temperature}°C</div>
          <p className="text-[10px] text-slate-500 mt-1 font-bold uppercase">Normal cycle</p>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-2 text-emerald-600 mb-2">
            <span className="text-[10px] font-black uppercase tracking-widest">Soil pH</span>
          </div>
          <div className="text-3xl font-black text-slate-800 tracking-tighter">{current.ph}</div>
          <p className="text-[10px] text-slate-500 mt-1 font-bold uppercase">Target: 6.5</p>
        </motion.div>
      </div>

      {/* Chart */}
      <div className="h-64 w-full">
        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 px-1">Analytical Trends</h3>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
            <XAxis dataKey="time" stroke="#94a3b8" fontSize={10} tickMargin={10} />
            <YAxis yAxisId="left" stroke="#3b82f6" fontSize={10} domain={[0, 100]} />
            <YAxis yAxisId="right" orientation="right" stroke="#f97316" fontSize={10} domain={[0, 50]} />
            <Tooltip
